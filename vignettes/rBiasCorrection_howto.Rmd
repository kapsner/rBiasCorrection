...
- the path to the `experimental` file, including it's filename  
- the path to the `calibration` file, including it's filename  
- `samplelocusname`: the name of the sample or locus under investigation  
- the `seed` argument shoud be set for reproducibility  
...
For demonstration purposes, we will here correct biases in only one CpG site. The example data is included in this R package. 

```{r include=FALSE}
cols <- c("sample_id", "CpG#1")
temp_file <- rBiasCorrection::example.data_experimental$dat[, cols, with = F]
data.table::fwrite(temp_file, "./experimental_data.csv")
cols <- c("true_methylation", "CpG#1")
temp_file <- rBiasCorrection::example.data_calibration$dat[, cols, with = F]
data.table::fwrite(temp_file, "./calibration_data.csv")
```

```{r}
experimental <- "./experimental_data.csv"
calibration <- "./calibration_data.csv"
```

# Conduct the correction of experimental biases  

The above defined variables can now be passed to the function `rBiasCorrection::biascorrection` in order to calculate the bias-corrected values of the experimental data.
...
Furthermore, the mean methylation percentages of all CpG columns are calculated for every provided file and stored in a new column *rowmeans*. 

Biases are calculated using two regression algorithms: *hyperbolic* and *cubic polynomial* regression. With the default settings, the general equations will be used for both algorithms.
However, an experimental feature exists, which can be accessed by using the argument `minmax = TRUE`. These regression equations are somehow data-dependend, assuming, that the minima and maxima of the provided calibration data are not biased at all (e.g. 100% actual methylation means 100% observed methylation).
...
# Data dependend regression equations (experimental feature) 
...
The correction algorithm to correct the biases can be chosen by setting the argument `correct_method` to either 'hyperbolic' or 'cubic'. If using the default setting 'best', the regression method will be selected for each CpG site based on best on the method, specified in the `selection_method` argument. 
...
-  **[name]_corrected_values_[timestamp].csv**: the bias corrected experimental data (this is the final results table of the samples under investigaion)  
...
-  [name]_corrected_calibrations_h_[timestamp].csv: the calibration data, which has been bias corrected using the hyperbolic regression parameters for all CpG sites  
-  [name]_corrected_calibrations_c_[timestamp].csv: the calibration data, which has been bias corrected using the cubic regression parameters for all CpG sites  
...
```{r}
filename <- list.files(csvdir)[grepl("regression_stats_[[:digit:]]", list.files(csvdir))]
...
![](./png/CDH1_CpG1_corrected_c.png){width=40%} ![](./png/CDH1_CpG1_corrected_h.png){width=40%}
![](./png/CDH1_error_CpG1_corrected_c.png){width=40%} ![](./png/CDH1_error_CpG1_corrected_h.png){width=40%}
